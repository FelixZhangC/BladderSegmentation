...
def train():
    ...
    # model
    G = ResUNet(in_channel=1,n_classes=3).to(device)
    ...
    for epoch in range(total_epoches):
        ...
        for real_imgs,real_labels in trainloader:
            ...
            seg_loss = WeightedDiceLoss(pred_labels,real_labels)
            seg_loss.backward()
        ...
        for imgs,labels in valloader:
            ...
            val_loss = WeightedDiceLoss(pred_val_labels,labels)
        ...
        print("epoch[%d/%d]  TrainDiceLoss :%.4f ; ValDiceLoss :%.4f ; Time: %s"%(epoch,total_epoches,ave_train_loss,ave_val_loss,present_time))
